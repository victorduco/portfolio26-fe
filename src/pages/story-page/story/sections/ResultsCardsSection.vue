<template>
  <section class="story-results-2">
    <div class="results-content">
      ...
      <p class="results-text results-intro">{{ introText }}</p>
      <div class="results-wrapper" ref="containerRef">
        <MediaLabel v-if="mediaLabel" :label="mediaLabel" tag="h3" />
        <div class="results-container" :style="{ '--bg-color': cardBackground, ...shadowVars }">
          <div class="results-grid">
            <div v-for="(result, index) in results" :key="index" :ref="(el) => (cardRefs[index] = el)" class="result-card">
      ...
      <p v-if="conclusionText" class="results-text results-conclusion">{{ conclusionText }}</p>
    </div>
  </section>
</template>
...
const updateParallax = () => {
  if (!containerRef.value || !isInViewport || isMobile.value) {
    if (isMobile.value) cardRefs.value.forEach((card) => card && (card.style.transform = "translateY(0)"));
  ...
  cardRefs.value.forEach((card, index) => {
    if (card) card.style.transform = `translateY(${scrollProgress * 100 * (parallaxSpeeds[index] || 0.5)}px)`;
  });
  ...
  if (containerRef.value) {
    observer = new IntersectionObserver(
      (entries) => entries.forEach((entry) => {
        ...
        isInViewport ? (!animationFrameId && (animationFrameId = requestAnimationFrame(updateParallax))) : (animationFrameId && (cancelAnimationFrame(animationFrameId), animationFrameId = null));
      }),
...
.results-text {
  font: var(--font-weight-regular) 16px/1.8 var(--font-family-base);
  margin: 0;
  opacity: 0.8;
}
.results-intro { margin-bottom: 48px; }
.results-conclusion { margin-top: 48px; }
.results-wrapper {
  width: 100vw;
  position: relative;
  left: 50%;
...
.result-card {
  ...
  background: #fff;
  border-radius: 24px;
  box-shadow: 0 3px 12px color-mix(in srgb, var(--shadow-saturated) var(--shadow-opacity), transparent);
  display: flex;
...
.result-label {
  font: var(--font-weight-bold) clamp(12px, 2.4vw, 14px)/1 var(--font-family-base);
...
.result-description {
  font: var(--font-weight-regular) 20px/1.5 var(--font-family-base);
  opacity: 0.8;
}
@media (max-width: 900px) {
  .results-grid { flex-direction: column; gap: 16px; }
  .results-container { padding: 24px 20px; aspect-ratio: auto; }
  .result-card { width: 100%; height: auto; min-height: 180px; border-radius: 16px; }
  .result-card-header { padding: 16px 20px 12px; }
  .result-card-header::after { left: 20px; right: 20px; }
  .result-card-inner { padding: 24px 20px; gap: 12px; }
  .result-value { font-size: clamp(24px, 5vw, 32px); }
  .result-value :deep(.rocket-icon) { width: 36px; height: 16px; margin: 0 8px; }
  .result-description { font-size: 16px; }
}
@media (max-width: 768px) {
  .story-results-2 { padding: 60px 0 24px; }
  .story-results-title { margin-bottom: 16px; }
  .results-intro { margin-bottom: 32px; }
  .results-container { padding: 16px 12px; }
  .results-grid { gap: 12px; }
  .result-card { min-height: 150px; border-radius: 12px; }
  .result-card-header { padding: 12px 16px 10px; }
  .result-card-header::after { left: 16px; right: 16px; }
  .result-card-inner { padding: 16px; gap: 8px; }
  .result-value { font-size: clamp(20px, 5vw, 28px); }
  .result-value :deep(.rocket-icon) { width: 28px; height: 12px; margin: 0 6px; }
  .result-description { font-size: 14px; line-height: 1.4; }
}
</style>
