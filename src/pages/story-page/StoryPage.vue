<template>
  <div :class="`story${storyId}-page`" :style="pageStyle">
    <div class="story-main-content">
      ...
      <section
        v-for="(contentSection, key) in storyContent"
        :key="`${storyId}-${key}`"
        :id="`story${storyId}-${key}`"
      ...
      >
        <div v-if="contentSection.heading" class="section-title">
          ...
          <h3
            :class="
              contentSection.heading.sectionTag
                ? 'story-heading-two-level'
                : 'story-heading-single'
            "
          >
            <template v-if="contentSection.heading.sectionTag">
              <span class="story-heading-subtitle">{{
                contentSection.heading.sectionTag
              }}</span>
              <span class="story-heading-main">{{
        ...
        <ContentSection
          v-for="(section, i) in contentSection.sections"
          :key="`${storyId}-${key}-${i}`"
          :heading="section.heading"
          :text-before="section.textBefore"
          :media="section.media"
          :text-after="section.textAfter"
          :background-color="config.videoBackground"
        />
      </section>
      <section v-if="config.results" :id="`story${storyId}-results`">
        <ResultsCardsSection
          ...
          :intro-text="config.resultsIntro"
          :conclusion-text="config.resultsConclusion"
          :card-background="config.videoBackground"
          :media-label="config.resultsMediaLabel"
        />
        <ResultsGridSection
          v-else
          :results="config.results"
          :results-note="config.resultsNote"
        />
      </section>
      <section v-if="config.simpleResults" :id="`story${storyId}-results`">
        <SimpleResultsSection
          :title="config.simpleResults.title"
          :text="config.simpleResults.text"
        />
      </section>
    ...
    <template v-if="config.nextStory">
      <NextStoryPlaque
        :story-number="storyNames[config.nextStory.storyId - 1]"
      />
...
const nextStoryComponent = computed(() => {
  ...
  return defineAsyncComponent(() =>
    import(`./story/sections/next-story/Story${id}NextStory.vue`)
  );
});
...
const navigationSections = computed(() => {
  ...
  Object.keys(storyContent.value).forEach((key) => {
    nav.push({
      ...
      label: sectionLabels[key] || key.charAt(0).toUpperCase() + key.slice(1),
    });
  });
  if (config.value.results || config.value.simpleResults)
    nav.push({ id: `story${props.storyId}-results`, label: "Results" });
  return nav;
});
</script>

<style scoped>
[class*="story"][class$="-page"] {
  width: 100%;
  position: relative;
  overflow-x: hidden;
}
