...
export function createCardStyle(
  ...
  return {
    ...
    border:
      intensity > 0
        ? `1px solid hsla(${tintHue}, 92%, 86%, ${
            borderAlpha * backgroundMultiplier * intensity
          })`
        : "none",

    backdropFilter:
      intensity > 0
        ? `blur(${options.glassBlur * intensity}px) saturate(${
            100 + (options.glassSaturation - 100) * intensity
          }%) brightness(${1 + (brightness - 1) * intensity}) contrast(${
            1 + (contrast - 1) * intensity
          })`
        : "none",

    backgroundImage:
      hasBackgroundImage && intensity > 0
        ? `linear-gradient(145deg,
          hsla(${tintHue}, 70%, ${Math.min(
            82,
            60 + tintOpacity * 35
          )}%, ${clamp(
